...
{
  int album = ALBUM_FIRST + (ALBUM_LAST-ALBUM_FIRST)*level/GAME_LEVELS;
...
static int move(void *null)
{
  ...
  if (TIMEOUT(key_time, TICKS_PER_SEC/8)) {
    ...
    /* F7 - jump to a random theme */
      ...
      key_time = game_clock;
      push_clock();
      play_music(MUSIC_FIRST + rand()%(MUSIC_LAST-MUSIC_FIRST+1), FALSE);
      pop_clock();
    }
  }
  ...
  if ((MAX(player1.level, player2.level) == GAME_LEVELS+1) &&
...
static void draw(void *null)
{
  ...
  if (change_background) {
    ...
    if ((scale > 0) && (background >= ALBUM_FIRST)) {
      ...
      draw_trans_sprite(virtual, bmp,
        GAME_SCREEN_W/2-scale/2,
        GAME_SCREEN_H/2-scale/2);
  ...
  /* draw the active theme? */
  if ((midi_pos >= 0) && (midi_pos < 6)) {
    int i, x, y, index, max;

    x = GAME_SCREEN_W/2;
    y = GAME_SCREEN_H-(8+2)*3;
